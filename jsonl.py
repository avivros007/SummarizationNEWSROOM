...
def which(pgm):
    path = _os.getenv('PATH')
    for p in path.split(_os.path.pathsep):
        p = _os.path.join(p, pgm)
        if _os.path.exists(p) and _os.access(p, _os.X_OK):
            return p

_has = {
    "zcat":  not not which("zcat"),
    "bzcat": not not which("bzcat"),
    "xzcat": not not which("xzcat"),
}
